...
- Added `omp config doctor`, a cross-layer config validation command that flags models.yml files the model registry rejects, model roles and fallback chains resolving to disabled providers, ambiguous bare model selectors served by multiple enabled providers, and leftover config for disabled providers ([#12163](https://github.com/can1357/oh-my-pi/pull/12163) by [@Gablinas](https://github.com/Gablinas)).
